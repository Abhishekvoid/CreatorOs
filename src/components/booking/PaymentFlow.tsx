  ...
  return (
    <div className="grid items-start gap-7 lg:grid-cols-[400px_1fr]">
        ...
        <div className="p-6">
          ...
          <dl className="mt-4 flex flex-col gap-2.5 border-t border-line pt-4 text-[13.5px]">
            ...
            <div className="mt-1 flex items-baseline justify-between border-t border-line pt-3.5">
              ...
              <dd className="text-[24px] font-bold tracking-tight">{inr(PRICE)}</dd>
            </div>
          </dl>
          ...
          <div className="mt-4.5 flex flex-col gap-2 border-t border-line pt-4">
            {["Instant confirmation after payment", "Free reschedule up to 24 hrs before", "Full refund if your first session disappoints"].map(
              (text) => (
                <div key={text} className="flex items-center gap-2 text-[12.5px] font-medium text-muted">
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.6} strokeLinecap="round" strokeLinejoin="round" className="size-3.5 shrink-0 text-ink-2" aria-hidden="true">
      ...
      <div className="rounded-[26px] border border-line bg-paper p-6.5 max-sm:p-5">
        <h2 className="text-[18px] font-bold tracking-tight">Choose how to pay</h2>
          ...
          {METHODS.map((m) => {
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="flex items-center gap-2 text-[15px] font-bold tracking-tight">
                      {m.label}
                      {m.id === "upi" && (
                        <span className="rounded-full bg-cream-2 px-2 py-0.5 text-[10.5px] font-semibold uppercase tracking-[0.08em] text-ink-2">
                          Recommended
                        </span>
                      )}
                    </span>
                    <span className="block text-[12.5px] font-medium text-muted">{m.note}</span>
                  </span>
                  <span
                ...
                {/* expanding panel */}
                <div className={`grid transition-[grid-template-rows] duration-300 ease-soft ${sel ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
                          ...
                          {BANKS.map((b) => {
                            ...
                            return (
                              <button
                                ...
                                className={`rounded-[12px] border-[1.5px] px-2 py-3 text-[13px] font-semibold transition-all duration-150 ease-soft ${
                                  selBank
                                    ? "border-ink bg-ink text-cream"
                                    : "cursor-pointer border-line text-ink-2 hover:border-ink"
                                }`}
                              >
                                {b}
                              </button>
